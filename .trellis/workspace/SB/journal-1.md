...
用户实测：改 SS 端口时按回车想退出，被随机改成 62537——端口提示的约定是「留空=随机」，而这一步没有取消键，只能 Ctrl-C。修复：choose_ss_port 加「0：取消」并返回专用状态码 2（enable_ss_entry 收到就中止，不再顺手生成密钥与随机端口）；change_ss_port 与 change_ports(hy2) 提示写明输入0取消并显式提示；上游地址/端口/密钥原本用 0 静默返回，改为打印取消提示并把菜单名从过期的「返回主菜单」改成「取消」；改密钥提示从「返回凭据菜单」改为「返回可选功能」。规范补：写活配置的取值提示必须有显式出口、空输入不算取消。测试：改端口取消后配置 sha256 不变且未提交、choose_ss_port 取消返回 2。
